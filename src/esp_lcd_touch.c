...
bool esp_lcd_touch_get_coordinates(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    for (int i = 0; (sw_adj_needed && i < *point_num); i++) {
        if (tp->config.flags.mirror_x && tp->set_mirror_x == NULL) x[i] = tp->config.x_max - x[i];
        if (tp->config.flags.mirror_y && tp->set_mirror_y == NULL) y[i] = tp->config.y_max - y[i];
...
esp_err_t esp_lcd_touch_get_button_state(esp_lcd_touch_handle_t tp, uint8_t n, uint8_t *state)
{
    ...
    assert(state != NULL);
    *state = 0;
    if (tp->get_button_state) return tp->get_button_state(tp, n, state);
...
esp_err_t esp_lcd_touch_set_swap_xy(esp_lcd_touch_handle_t tp, bool swap)
{
    ...
    if (tp->set_swap_xy) return tp->set_swap_xy(tp, swap);
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_get_swap_xy(esp_lcd_touch_handle_t tp, bool *swap)
{
    assert(tp != NULL);
    assert(swap != NULL);
    if (tp->get_swap_xy) return tp->get_swap_xy(tp, swap);
    else *swap = tp->config.flags.swap_xy;
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_set_mirror_x(esp_lcd_touch_handle_t tp, bool mirror)
{
    ...
    if (tp->set_mirror_x) return tp->set_mirror_x(tp, mirror);
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_get_mirror_x(esp_lcd_touch_handle_t tp, bool *mirror)
{
    assert(tp != NULL);
    assert(mirror != NULL);
    if (tp->get_mirror_x) return tp->get_mirror_x(tp, mirror);
    else *mirror = tp->config.flags.mirror_x;
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_set_mirror_y(esp_lcd_touch_handle_t tp, bool mirror)
{
    ...
    if (tp->set_mirror_y) return tp->set_mirror_y(tp, mirror);
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_get_mirror_y(esp_lcd_touch_handle_t tp, bool *mirror)
{
    assert(tp != NULL);
    assert(mirror != NULL);
    if (tp->get_mirror_y) return tp->get_mirror_y(tp, mirror);
    else *mirror = tp->config.flags.mirror_y;
    return ESP_OK;
}
...
esp_err_t esp_lcd_touch_register_interrupt_callback(esp_lcd_touch_handle_t tp, esp_lcd_touch_interrupt_callback_t callback)
{
    ...
    assert(tp != NULL);
    if (tp->config.int_gpio_num == GPIO_NUM_NC) return ESP_ERR_INVALID_ARG;
